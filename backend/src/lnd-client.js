...
class LND {
    ...
    payInvoice(paymentRequest) {
        return this._exec(`payinvoice --force --pay_req="${paymentRequest}"`);
    }

    // 查询发票
    lookupInvoice(rHash) {
        return this._exec(`lookupinvoice --rhash="${rHash}"`);
    }

    // 解码付款请求
    decodePayReq(paymentRequest) {
        return this._exec(`decodepayreq "${paymentRequest}"`);
    }

    // 签名消息
    signMessage(msg) {
        return this._exec(`signmessage "${msg}"`);
    }

    // 验证签名
    verifyMessage(msg, signature, pubkey) {
        return this._exec(`verifymessage --msg="${msg}" --sig="${signature}" --pubkey="${pubkey}"`);
    }
